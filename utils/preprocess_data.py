...
def process_file_group(group_id, input_dir, output_dir, fs, use_gpu=False, gpu_id=0):
    ...
    try:
        ...
        # 加载原始信号
        farend_path = os.path.join(input_dir, 'farend', f'{group_id}_farend.wav')
        mic_path = os.path.join(input_dir, 'mic', f'{group_id}_mic.wav')
        nearend_path = os.path.join(input_dir, 'nearend', f'{group_id}_nearend.wav')
        ...
        sf.write(os.path.join(output_dir, 'farend', f'{group_id}_farend.wav'), farend, fs)
        sf.write(os.path.join(output_dir, 'mic', f'{group_id}_mic.wav'), mic, fs)
        sf.write(os.path.join(output_dir, 'nearend', f'{group_id}_nearend.wav'), nearend, fs)
        sf.write(os.path.join(output_dir, 'error', f'{group_id}_error.wav'), error, fs)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_workers', type=int, default=4, help='处理的CPU工作进程数')
    parser.add_argument('--use_gpu', default=True, help='是否使用GPU加速')
